...
### Ride Frequency by Gender
```{r}
unknownBikers <- citibike[citibike$gender == 0,]
...
### Routes originating longest rides

First we want to analyze the trip duration data
```{r}
...
It is extremely apparent that there are a lot of outliers that do not make sense in the context of this data. For example, the maximum trip duration lasted `r max(dur)` minutes - which clearly makes no sense. 
...
```{r}
filteredDuration <- citibike[citibike$tripduration <= 120*60 & citibike$tripduration >= 60,]
```
...
```{r}
avgDurationByStation <- aggregate(x = filteredDuration$tripduration, by = filteredDuration[,c("start.station.id", "end.station.id")], FUN = mean)
avgDurationByStation <- merge(x = avgDurationByStation, y = uniqueStation, by.x = 2, by.y = 1, sort = FALSE)
colnames(avgDurationByStation)[4] <- "end.latitude"
colnames(avgDurationByStation)[5] <- "end.longitude"
colnames(avgDurationByStation)[6] <- "end.station_name"

avgDurationByStation <- merge(x = avgDurationByStation, y = uniqueStation, by.x = 2, by.y = 1, sort = FALSE)
colnames(avgDurationByStation)[7] <- "start.latitude"
colnames(avgDurationByStation)[8] <- "start.longitude"
colnames(avgDurationByStation)[9] <- "start.station_name"

#sort by average duration
avgDurationByStation <- avgDurationByStation[order(avgDurationByStation$x),]
...
nymap <- get_map(location = "New York", zoom = 12, color = 'bw')
shortRouteMap <- ggmap(nymap)

for (i in 1:length(shortestBikeRoutes)){
  from = as.character(paste("New York,",shortestBikeRoutes[i,"start.station_name"]))
  to = as.character(paste("New York,", shortestBikeRoutes[i,"end.station_name"]))
  route_df = route(from, to, structure = "route")
  shortRouteMap <- shortRouteMap + geom_path(
...
longRouteMap <- ggmap(nymap)

for (i in 1:length(shortestBikeRoutes)){
  from = as.character(paste("New York,",longestBikeRoutes[i,"start.station_name"]))
  to = as.character(paste("New York,", longestBikeRoutes[i,"end.station_name"]))
  route_df = route(from, to, structure = "route")
  longRouteMap <- longRouteMap + geom_path(
